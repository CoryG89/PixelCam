var CamMan = (function () {
    ...
    CamMan.prototype.createCanvas = function createCanvas(onFrame) {
        var canvas = null;
    ...
    var perFrameEvtPrefix = 'canvas';

    /** Inject a canvas element with video output into a container element */
    CamMan.prototype.getCanvas = function getCanvas(container, onFrame) {
        var element = document.querySelector('#' + container);
        
        if (element) {
            if (onFrame) {
                var eventName = perFrameEvtPrefix + this.canvasStore.length;
                this.on(eventName, onFrame.bind(this));
            }
            return element.appendChild(this.createCanvas(onFrame));
        }
    };
    ...
    CamMan.prototype.updateCanvas = function updateCanvas() {
        for (var i = 0; i < this.canvasStore.length; i++) {
            ...
            this.trigger(perFrameEvtPrefix + i, canvas);
        }
